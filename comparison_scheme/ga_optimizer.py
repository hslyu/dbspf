...
def decode_gene(X):
    ...
    list_ue_alpha = (
        np.arange(param.num_ue).reshape([-1, 1]) == list_ue_alpha_encoded
    ).astype(int)
    ...
    list_ue_power = (
        np.apply_along_axis(proportion_array, 1, list_ue_power_encoded)
        * param.max_ue_power_mW
    )
    #    print(list_ue_alpha)
        ...
        if list_ue_mode[i] == 1:
            for j, subcarrier in enumerate(ue.list_subcarrier_UBS):
                if list_ue_alpha[i, j] == 1:
                    ...
                    power_UBS_GBS = list_UBS_power[j] * dB2orig(
                        UBS.list_subcarrier[j].channel
                    )
                    ...
                    if (
                        power_ue_UBS / dB2orig(param.noise) < param.SNR_threshold
                        or power_UBS_GBS / (dB2orig(param.ICI) + dB2orig(param.noise))
                        < param.SNR_threshold
                    ):
                    ...
                    snr = power / noise
                    carrier_rate += (
                        param.subcarrier_bandwidth * math.log2(1 + snr) / 2
                    )  # Datarate of k-th subcarrier for i-th user
        ...
        #                    print(carrier_rate)
            ...
            for j, subcarrier in enumerate(ue.list_subcarrier_GBS):
                if list_ue_alpha[i, j] == 1:
                    ...
                    if (
                        power / (dB2orig(param.noise) + dB2orig(param.ICI))
                        < param.SNR_threshold
                    ):
                        continue

                    carrier_rate += (
                        param.subcarrier_bandwidth
                        * math.log2(1 + power / dB2orig(param.noise))
                        / 2
                    )
                    carrier_rate += (
                        param.subcarrier_bandwidth
                        * math.log2(
                            1 + power / (dB2orig(param.noise) + dB2orig(param.ICI))
                        )
                        / 2
                    )
                    sumrate += carrier_rate
...
num_genes = (
    param.num_ue
    + param.num_subcarriers
    + param.num_ue * param.num_subcarriers
    + param.num_subcarriers
    + 1
    + 1
    + 1
...
ue_power_bound = {
    "low": 0,
    "high": 10 + 1,
}  # int, The number 0 and 100 are not power bounds. It is softmax ratio
UBS_power_bound = {
    "low": 0,
    "high": 10 + 1,
}  # int, The number 0 and 100 are not power bounds. It is softmax ratio
